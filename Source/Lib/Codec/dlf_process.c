...
#if OPT_FRAME_DLF
            if (pcs->zero_filt_sse == -1 && (frm_hdr->loop_filter_params.filter_level[0] || frm_hdr->loop_filter_params.filter_level[1])) {
...
#if OPT_FRAME_DLF
            if (pcs->best_filt_sse == -1 && (frm_hdr->loop_filter_params.filter_level[0] || frm_hdr->loop_filter_params.filter_level[1])) {
                pcs->best_filt_sse = picture_sse_calculations(pcs, recon_buffer, /*plane*/0);
            }
            pcs->dlf_dist_dev =
                pcs->zero_filt_sse == 0 || !(frm_hdr->loop_filter_params.filter_level[0] || frm_hdr->loop_filter_params.filter_level[1]) ?
                0 : 1000 - ((1000 * pcs->best_filt_sse) / pcs->zero_filt_sse);
#endif
        }
